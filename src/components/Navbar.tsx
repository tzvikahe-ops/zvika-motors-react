...
export default function Navbar({ currentPage, setPage }: NavbarProps) {
  ...
  return (
    ...
    >
      <div className="max-w-[1200px] mx-auto px-6 flex items-center justify-between h-[68px]">
        <div onClick={() => setPage("home")} className="cursor-pointer flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-primary-foreground/40 text-[10px] font-medium leading-tight">אור-צת שירותי רכב · מאז 1993</span>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-1">
          {links.map(({ label, page }) => (
            <button
              ...
              className={`bg-transparent border-none cursor-pointer text-[13px] px-3 py-2 rounded-md transition-all duration-200 ${
                currentPage === page
                  ? "text-accent-foreground bg-brand-red font-bold"
                  : "text-primary-foreground/60 font-medium hover:text-primary-foreground hover:bg-primary-foreground/[0.06]"
              }`}
            >
              {label}
            </button>
        ...
        <a
          href="tel:02-6514446"
          className="hidden md:flex items-center gap-2 bg-brand-red text-accent-foreground border-none rounded-md px-5 py-2 text-[13px] font-bold cursor-pointer hover:bg-brand-red-hover transition-all duration-200 no-underline"
        >
          02-6514446
        </a>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-surface-darker px-4 py-3 flex flex-col gap-0.5 border-t border-primary-foreground/[0.06]">
          {links.map(({ label, page }) => (
            <button
              key={label}
              ...
              className={`bg-transparent border-none cursor-pointer text-[15px] text-right py-3 rounded-md px-4 transition-all duration-200 ${
                currentPage === page
                  ? "text-accent-foreground font-bold bg-brand-red"
