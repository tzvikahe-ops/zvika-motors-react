...
export default function ServicesSection({ setPage }: { setPage: (p: Page) => void }) {
  return (
    <section id="services" className="py-16 md:py-28 px-5 sm:px-6 bg-surface-darker relative" dir="rtl" aria-label="שירותי המוסך">
      ...
      <div className="max-w-[1100px] mx-auto relative z-10">
        <div className="flex items-baseline justify-between mb-10 md:mb-12">
          <div>
            ...
            <h2 className="text-[24px] sm:text-[28px] md:text-[34px] font-black text-primary-foreground tracking-[-0.03em] leading-[1.12]">
              מה אנחנו עושים
            </h2>
          </div>
          <button
            onClick={() => setPage("services")}
            className="text-primary-foreground/30 text-[12px] font-medium hover:text-primary-foreground/60 cursor-pointer bg-transparent border-none transition-colors duration-200 hidden sm:block"
          >
            כל השירותים ←
          </button>
        </div>
        ...
        <div className="border border-primary-foreground/[0.08] mb-px overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1.1fr]">
            <div className="bg-surface-dark p-6 sm:p-8 md:p-10 flex flex-col justify-center order-2 md:order-1">
              <div className="flex items-center gap-2 mb-4">
                <WrenchSVG />
                <span className="text-primary-foreground/20 text-[10px] font-bold tracking-wider">שירות מרכזי</span>
              </div>
              <h3 className="font-black text-[18px] sm:text-[20px] md:text-[22px] text-primary-foreground tracking-[-0.02em] mb-3">מכונאות רכב כללית</h3>
              <p className="text-primary-foreground/40 text-[13px] md:text-[14px] leading-[1.85] max-w-[400px] mb-6">
                טיפולים שוטפים, החלפת שמנים, תיקוני מנוע ובלמים. היסודות שמאריכים את חיי הרכב שלכם, בביצוע מקצועי ואמין.
              </p>
              <button
                onClick={() => setPage("services")}
                className="btn-text group w-fit"
              >
